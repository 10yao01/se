...
CREATE TABLE IF NOT EXISTS `fertile` (
  ...
  `irrigate_time` date NOT NULL COMMENT '灌溉时间',
  `fertile_time` date NOT NULL COMMENT '施肥时间',
  PRIMARY KEY (`f_id`) USING BTREE,
...
CREATE TABLE IF NOT EXISTS `goods` (
  ...
  `score` double DEFAULT NULL COMMENT '平均评分',
  `raters_num` int DEFAULT NULL COMMENT '评分人数',
  PRIMARY KEY (`g_id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- 正在导出表  se.goods 的数据：~4 rows (大约)
REPLACE INTO `goods` (`g_id`, `g_name`, `stock`, `sale`, `price`, `score`, `raters_num`) VALUES
...
-- 正在导出表  se.poultry 的数据：~4 rows (大约)
