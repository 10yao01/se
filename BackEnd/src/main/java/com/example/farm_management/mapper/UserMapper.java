...
public interface UserMapper {
    ...
    @Insert("insert into user(u_id,pwd,id_type,name,gender,age,tel)" +
            "values (#{uid},AES_ENCRYPT(#{pwd}, 'secret key'),#{idtype},#{name},#{gender},#{age},#{tel})")
    ...
    @Select("select * from user where u_id=#{uid} and pwd = AES_ENCRYPT(#{pwd}, 'secret key')")
    User getByNAP(User user);

    @Select("select id_type from user where u_id=#{uid}")
